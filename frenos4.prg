// Programa   : FRENOS4
// Fecha/Hora : 18/03/2024 
// Propósito  : https://github.com/AdaptaProERP/excel/blob/main/productos4.csv
// Creado Por :
// Llamado por:
// Aplicación :
// Tabla      :
...
FUNCTION MAIN()
  LOCAL cFile:="C:\frenos\productos4.csv"
...
//  ADEPURA(oXls:aData,{|a,n| Empty(a[3])})
 ...
 // SQLDELETE("DPINV","INV_USO"+GetWhere("=","LISTA4"))
  nIni:=SQLGETMAX("DPINV","INV_VOLUME","INV_USO"+GetWhere("=","LISTA4"))
  ...
  WHILE !oXls:Eof() 
     ...
     cDescri:=ALLTRIM(oXls:COL_L)

     IF !Empty(cDescri) .AND. !ISSQLFIND("DPMARCAS","MAR_DESCRI"+GetWhere("=",cDescri))

          cCodMar:=SQLINCREMENTAL("DPMARCAS","MAR_CODIGO",NIL,NIL,NIL,.T.,4)

          EJECUTAR("CREATERECORD","DPMARCAS",{"MAR_CODIGO","MAR_DESCRI","MAR_ACTIVO"},;
                                             {cCodMar     ,cDescri     ,.T.         },NIL,.T.,;
                                             "MAR_DESCRI"+GetWhere("=",cDescri))
     ENDIF
