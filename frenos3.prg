...
// Propósito  : https://github.com/AdaptaProERP/excel/blob/main/productos3.csv
// Creado Por :
// Llamado por:
// Aplicación :
// Tabla      :
...
FUNCTION MAIN()
  LOCAL cFile:="C:\frenos\productos3.csv"
  LOCAL oNew:=OpenTable("SELECT * FROM DPINV",.F.),cWhere
  LOCAL cCodInv,cCodGru,cCodMar,cDescri:="ESTOPERAS DE RUEDA"
  LOCAL cUndMed,nCxUnd,cCodMon:="DBC",nUndMed:=1,cUndGru:="CAJA"
  LOCAL nContar:=0,nIni:=SQLGETMAX("DPINV","INV_VOLUME","INV_USO"+GetWhere("=","LISTA3"))
  LOCAL nPrecioH,nPrecioI,nCosto,cDescri
  LOCAL oXls   :=NIL
...
//  SQLDELETE("DPINV","INV_USO"+GetWhere("=","LISTA3"))
  ...
  IF Empty(cCodGru)
     ...
     cCodGru:=SQLINCREMENTAL("DPGRU","GRU_CODIGO",NIL,NIL,NIL,.T.,10)
...
// ViewArray(oXls:aData)
  ...
  WHILE !oXls:Eof()
    ...
    oNew:Replace("INV_OBS1"  ,oXls:COL_D)
    oNew:Replace("INV_OBS2"  ,oXls:COL_E)
    oNew:Replace("INV_OBS3"  ,oXls:COL_F)

    oNew:Replace("INV_UTILIZ","Venta")
    oNew:Replace("INV_ESTADO","A"    )
    oNew:Replace("INV_APLICA","T"    )
    oNew:Replace("INV_VOLUME",oXls:RecNo()) 
    oNew:Replace("INV_USO"   ,"LISTA3")
